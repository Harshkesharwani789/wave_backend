  ...
  try {
    ...
    const hierarchyData = await Promise.all(
      categories.map(async (category) => {
        ...
        console.log(
          `Found ${services.length} services for category ${category.name}:`,
          services
        );
        ...
        const servicesWithSubservices = await Promise.all(
          services.map(async (service) => {
            ...
            console.log(
              `Found ${subServices.length} subservices for service ${service.name}:`,
              subServices
            );
...
const getCategories = async (req, res) => {
  try {
    const categories = await ServiceCategory.find({ isActive: true }).populate({
      path: "subcategories",
      match: { isActive: true },
      populate: {
        path: "services",
...
const getAllSubServices = async (req, res) => {
  try {
    const subservices = await SubService.find({ isActive: true }).populate({
      path: "service",
      populate: {
        path: "subCategory",
        populate: {
...
const viewPartnerCart = async (req, res) => {
  try {
    ...
    if (!booking) {
      return res
        .status(403)
        .json({ message: "Unauthorized to view this cart" });
    }
...
const approvePartnerCart = async (req, res) => {
  try {
    ...
    if (!booking) {
      return res
        .status(403)
        .json({ message: "Unauthorized: This booking does not belong to you" });
    }
    ...
    if (!booking.cart || booking.cart.length === 0) {
      return res
        .status(400)
        .json({ message: "No products in cart for this booking" });
    }
    ...
    return res.status(200).json({
      success: true,
      message:
        "Products approved, inventory updated, and cart saved permanently",
